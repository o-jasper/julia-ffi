...
function FFI_Info(on_file::String, opts::Options)
    ...
    return FFI_Info(lib_var, on_file, namer,file_namer,lib_namer,
...
function default_pprint(file::String, info)
    on_file = info.on_file
    @with from = stream_from_string(readall(`gcc -E $file`)) begin
        @with to = open(info.file_namer(file), "w") begin
            info.on_file = file
...
# TODO assert integer, filename, integer,integer
    src_file = list[2]
    if info.follow_unseen && isfile(src_file) && 
       !get(info.seen_files, src_file, false) #Read files not seen before.
        assign(info.seen_files, true, src_file)
      #TODO want to read stdout from parallel program, this is incredibly crude.
        default_pprint(src_file, info)
    end   
